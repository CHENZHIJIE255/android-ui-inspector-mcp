 ...
 */
export function findViews(nodes, params, path = "") {
    const result = [];
...
function matches(node, params) {
    ...
    if (params.displayed !== undefined) {
        ...
        if (params.displayed !== (hasSize && node.enabled))
            return false;
    }
    // String / string[] fields — each can be a single string or an array (OR).
    // 字符串字段，支持单字符串或字符串数组（数组内 OR）
    if (!matchField(node.text, toArray(params.text), (a, b) => a === b))
        return false;
    if (!matchField(node.text, toArray(params.text_contains), (a, b) => a.toLowerCase().includes(b.toLowerCase())))
        return false;
    if (params.text_regex !== undefined) {
        try {
            if (!new RegExp(params.text_regex).test(node.text))
                return false;
        }
        catch {
    ...
    if (!matchField(node.content_desc, toArray(params.content_desc), (a, b) => a === b))
        return false;
    if (!matchField(node.class_name, toArray(params.class_name), (a, b) => a === b || a.endsWith(b)))
        return false;
    if (!matchField(node.resource_id, toArray(params.resource_id), (a, b) => a === b || a.endsWith(b)))
        return false;
    if (!matchField(node.package_name, toArray(params.package_name), (a, b) => a === b))
        return false;
    // Boolean fields / 布尔字段
    if (params.clickable !== undefined && node.clickable !== params.clickable)
        return false;
    if (params.enabled !== undefined && node.enabled !== params.enabled)
        return false;
    if (params.focused !== undefined && node.focused !== params.focused)
        return false;
    if (params.checkable !== undefined && node.checkable !== params.checkable)
        return false;
    if (params.checked !== undefined && node.checked !== params.checked)
        return false;
    if (params.selected !== undefined && node.selected !== params.selected)
        return false;
    if (params.scrollable !== undefined && node.scrollable !== params.scrollable)
        return false;
    // has_* boolean fields — check for non-empty string / has_* 布尔字段——检查字符串非空
    if (params.has_text !== undefined && (node.text === "") === params.has_text)
        return false;
    if (params.has_content_desc !== undefined && (node.content_desc === "") === params.has_content_desc)
        return false;
    if (params.has_resource_id !== undefined && (node.resource_id === "") === params.has_resource_id)
        return false;
    // Cross-field $or — at least one sub-query must match
    // 跨字段 $or —— 至少一个子查询匹配
    if (params.$or !== undefined && params.$or.length > 0) {
        if (!params.$or.some(sub => matches(node, sub)))
            return false;
    }
    return true;
}
 ...
 */
function matchField(fieldValue, candidates, compare) {
