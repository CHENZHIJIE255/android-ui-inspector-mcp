...
const MESSAGES = {
    en: {
        // ── ADB errors / ADB 错误 ──
        "adb.not_found": "ADB not found in PATH. Install Android SDK Platform Tools:\n" +
            "  https://developer.android.com/tools/releases/platform-tools\n" +
            "Then ensure adb is in your PATH.",
        "device.not_found": "No Android device found. Please connect a device or start an emulator.",
        "device.invalid_state": 'Device "{serial}" is not in "device" state.',
        "device.validate_failed": 'Failed to validate device "{serial}": {reason}',
        // ── ViewTree errors / 视图树错误 ──
        "viewtree.no_root": "No view root found in dump.",
        // ── JDWP errors / JDWP 错误 ──
        "jdwp.handshake_failed": "JDWP handshake failed — target may not be a Java process.",
        // ── General / 通用 ──
        "tool.unknown": 'Unknown tool: {name}',
    },
    zh: {
        "adb.not_found": "PATH 中未找到 ADB。请安装 Android SDK Platform Tools：\n" +
            "  https://developer.android.com/tools/releases/platform-tools\n" +
            "然后将 adb 加入 PATH。",
        "device.not_found": "未找到 Android 设备，请连接设备或启动模拟器。",
        "device.invalid_state": '设备 "{serial}" 状态不是 "device"。',
        "device.validate_failed": '验证设备 "{serial}" 失败：{reason}',
        "viewtree.no_root": "导出的视图层次结构中未找到根节点。",
        "jdwp.handshake_failed": "JDWP 握手失败——目标可能不是 Java 进程。",
...
export function detectLocale() {
    for (const envVar of ["LC_MESSAGES", "LANG", "LANGUAGE"]) {
        ...
        if (val.toLowerCase().includes("zh"))
            return "zh";
...
export function listLocales() {
    return Object.keys(LOCALE_NAMES).map((id) => ({
        id,
