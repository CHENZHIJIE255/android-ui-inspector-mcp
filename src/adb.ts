 ...
 */
export function checkAdbAvailable(locale?: Locale): void {
...
let _deviceSerial: string | undefined;
 ...
 */
export function findActiveDevice(locale?: Locale): string | undefined {
  const lang = locale ?? detectLocale();
...
export function ensureAdbAvailable(serial?: string, locale?: Locale): string {
  ...
  if (_deviceSerial) return _deviceSerial;
...
export function selectDevice(serial: string, locale?: Locale): void {
  ...
  try {
    ...
    if (e.message?.includes?.(t(lang, "device.invalid_state", { serial }).slice(0, 10))) throw e;
 ...
 */
export function runAdbRaw(args: string[]): string {
 ...
 * / 获取当前活跃的设备序列号。
 */
export function getActiveDeviceSerial(): string | undefined {
  return _deviceSerial;
}
 ...
 */
export function dumpViewTreeXml(serial?: string): string {
 ...
 */
export function getPackageName(serial: string, pid: number): string | undefined {
  try {
 ...
 */
export function forwardJdwp(
  serial: string | undefined,
  localPort: number,
  pid: number,
): { serial: string; localPort: number } {
  const resolvedSerial = ensureAdbAvailable(serial);
 ...
 * / 移除先前设置的 ADB 端口转发。
 */
export function removeForward(serial: string, localPort: number): void {
  try {
