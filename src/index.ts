 ...
 */
function stringOrArraySchema(description: string) {
  return {
    oneOf: [
...
/**
 * Walk tree and collect summary stats, returning a compact view for AI consumption
 * (avoids truncation from huge full-tree JSON).
 * / 遍历树、收集摘要统计，返回紧凑视图供 AI 使用（避免全量 JSON 被截断）。
 */
...
function summarizeTree(roots: ViewNode[]): TreeSummary {
  let totalNodes = 0;
  let maxDepth = 0;
  ...
  // / 第一遍：完整遍历全部节点，精确统计
  function countAll(nodes: ViewNode[], depth: number) {
    for (const n of nodes) {
      totalNodes++;
      maxDepth = Math.max(maxDepth, depth);
      classCount[n.class_name] = (classCount[n.class_name] || 0) + 1;
      if (n.clickable) stats.clickable++;
      if (n.enabled) stats.enabled++;
      if (n.scrollable) stats.scrollable++;
      if (n.text) stats.has_text++;
      if (n.content_desc) stats.has_content_desc++;
      if (n.resource_id) stats.has_resource_id++;
      if (n.package_name) apps.add(n.package_name);
      if (depth === 0) screenRes = `${n.bounds.width}x${n.bounds.height}`;
      if (n.children) countAll(n.children, depth + 1);
    }
  }
  countAll(roots, 0);
  ...
  function buildTree(nodes: ViewNode[], depth: number, prefix: string) {
 ...
 */
async function queryViewTree(params: FindParams, deviceSerial?: string, locale?: Locale): Promise<ViewTreeResult> {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
    case "dump_view_tree": {
      const dumpArgs = (args ?? {}) as Record<string, unknown>;
      const pkgFilter = dumpArgs.package_name as string | undefined;
      const dumpSerial = dumpArgs.device_serial as string | undefined;
      const lang = resolveLocale(dumpArgs.language as string | undefined);
      const params: FindParams = {};
      if (pkgFilter) params.package_name = pkgFilter;
      if (dumpSerial) params.device_serial = dumpSerial;
      ...
      if (dumpArgs.detailed === true) {
        ...
        const result: ViewTreeResult = { nodes: filtered, total_count: filtered.length, elapsed_ms: Date.now() - start };
    ...
    case "find_views": {
      const findArgs = (args ?? {}) as Record<string, unknown>;
      const findSerial = findArgs.device_serial as string | undefined;
      const lang = resolveLocale(findArgs.language as string | undefined);
      const params = findArgs as FindParams;
      if (params.device_serial) delete params.device_serial;
      if ((params as any).language) delete (params as any).language;
    ...
    case "jdwp_connect": {
      const jdwpArgs = (args ?? {}) as Record<string, unknown>;
      const { pid, port } = jdwpArgs as { pid: number; port?: number };
      const jdwpSerial = jdwpArgs.device_serial as string | undefined;
      const lang = resolveLocale(jdwpArgs.language as string | undefined);
      try {
